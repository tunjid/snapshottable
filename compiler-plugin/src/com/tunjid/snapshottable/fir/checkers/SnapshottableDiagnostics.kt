...
internal object SnapshottableDiagnostics : KtDiagnosticsContainer() {
    ...
    object Renderers : BaseDiagnosticRendererFactory() {
        override val MAP: KtDiagnosticFactoryToRendererMap by KtDiagnosticFactoryToRendererMap("Snapshottable sandbox errors") {
            ...
            it.put(NOT_SNAPSHOTTABLE_SPEC, "'@SnapshotSpec' requires final classes.")

            it.put(NO_SNAPSHOTTABLE_SPEC, "'@Snapshottable' interface does not have an accompanying '@SnapshotSpec'.")
            it.put(NO_SNAPSHOTTABLE_INTERFACE, "'@SnapshotSpec' is not nested in discoverable '@Snapshottable' parent interface.")

            it.put(NO_PRIMARY_CONSTRUCTOR, "'@SnapshotSpec' requires a primary constructor.")
            it.put(PRIVATE_CONSTRUCTOR, "'@SnapshotSpec' requires a public constructor.")
            it.put(ILLEGAL_VISIBILITY_MODIFIER, "'@SnapshotSpec' cannot have non public fields in it's primary constructor.")
        }
    }
}
